...
fn print_windows(monitor_window_collections: Vec<lib::MonitorWindowCollection>) {
    ...
    let current_window_handle = lib::get_current_monitor_window_collections(&monitor_window_collections);

    for collection in &monitor_window_collections {
        println!("Monitor {}: left={}, top={}, right={}, bottom={}", collection.monitor.id, collection.monitor.bounds.left, collection.monitor.bounds.top, collection.monitor.bounds.right, collection.monitor.bounds.bottom);

        for idx in 0..collection.windows.len() {
            let window = &collection.windows[idx];
            if window.is_visible() {
                println!("\t{}-{}: {}", collection.monitor.id, idx, collection.windows[idx].title);
            }
...
fn move_window(monitor_window_collections: Vec<lib::MonitorWindowCollection>, window_title: &str, monitor_id: u32) {
    let monitor = match monitor_window_collections.iter().find(|collection| collection.monitor.id == monitor_id){
    ...
    for collection in monitor_window_collections {
        for idx in 0..collection.windows.len() {
            ...
            if window.title.to_lowercase().contains(&window_title) || window_id.to_lowercase() == window_title {
                println!("Moving window '{}' to monitor {}", window.to_string(), monitor_id);
                lib::move_window_to_monitor(&window, &monitor);
                return;
            }
        }
    }
    println!("Window '{}' not found on monitor {}", window_title, monitor_id);
}
fn print_help() {
    println!("Usage:");
...
fn main() {
    ...
    match option {
        ...
        Options::Move(window_title, monitor_id) => move_window(monitor_window_collections, &window_title, monitor_id),
    }
}
